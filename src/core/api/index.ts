...
export default {
  ...
  requestNowPlaying: (page: number, sort: MovieSortType | undefined, search: string) => {
    ...
    return requestApi.get<MovieType[]>(
  ...
  requestPopular: (page: number, search: string) => requestApi.get<MovieType[]>(`/discover/movie?include_adult=false&include_video=false&language=en-US&page=${page}&sort_by=popularity.desc&with_keywords=${search}`),

  // DETAILS
  requestMovieDetails: (id: number) => requestApi.get<R>(`/movie/${id}`),
  requestMovieCredits: (id: number) => requestApi.get<R>(`/movie/${id}/credits`),
  requestMovieRecommendations: (id: number) => requestApi.get<MovieType[]>(`/movie/${id}/recommendations`),
